...
double method_1(int, double, double, float, float, float); // Прототип функции вычисляющей корни по заданному методу
...
void inputAndFails(int *, int *, double *, double *, float *, float *, float *, double *,double *); 

main(){
  srand(time(NULL)); // Генерация случайных чисел будет зависеть от времени
  
  int findMethod, eqNum;
  double k1, k2, E,E2, a, b, result, xn;

  inputAndFails(&findMethod, &eqNum, &k1, &k2, &E, &a, &b, &E2, &xn);
  if(findMethod == 1)
...
double method_1(int eN, double k1, double k2, float E, float a, float b){
...
void inputAndFails(int *fM, int *eN, double *k1, double *k2, float *E, float *a, float *b, double *E2, double *xn){
  float cache;
  // Выбор метода
